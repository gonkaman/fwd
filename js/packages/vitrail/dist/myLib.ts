...
export const area = createDOMAdapter<
    ...
    undefined, Document, 
    string
>('abbr', htmlNodeFactory, noNodeConnector, formatAdapterArgs);
...
export const slot = createDOMAdapter<
    HTMLElement, Document, 
...
export const var = createDOMAdapter<
    HTMLElement, Document, 
    HTMLElement, Document, 
    Text | HTMLElement, Document, 
    string
...
export const getClassName = <T extends Element, U extends Document>(key?: string): Filter<[T,U],[string, unknown][]> => getAttr('className', key);
...
export const get$for = <T extends HTMLLabelElement | HTMLOutputElement, U extends Document>(key?: string): Filter<[T,U],[string, unknown][]> => getAttr('$for', key);
...
export const dispatch = <T extends EventTarget, U extends Document>(name: string): NodeTask<T,U> => [
    (entry: [T,U]) => {
        entry[0].dispatchEvent(name);
        return entry;
    }
];
